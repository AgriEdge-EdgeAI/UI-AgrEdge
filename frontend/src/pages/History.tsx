...
// ML Prediction vs Actual Usage Data
...
const savingsAmount = totalPredicted - totalActual;
const savingsPercent = Math.round((savingsAmount / totalPredicted) * 100);
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2, mb: 4 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          <Button variant="outlined" startIcon={<PictureAsPdfIcon />} sx={{ borderRadius: 2, borderColor: '#f97316', color: '#f97316', '&:hover': { transform: 'translateY(-2px)' } }}>
            PDF Report
          </Button>
        </Box>
      </Box>
      ...
      <Grid container spacing={3}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2, mb: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <Avatar sx={{ bgcolor: `rgba(13,107,58,0.1)`, width: 40, height: 40 }}>
                  <FilterListIcon sx={{ color: PRIMARY_GREEN }} />
                </Avatar>
                ...
                {SEVERITY_FILTERS.map(f => (
                  <Chip
                    ...
                    sx={{
                      ...
                      bgcolor: filter === f ? PRIMARY_GREEN : 'transparent',
              ...
              <Table>
                <TableHead>
                  <TableRow sx={{ bgcolor: '#f8fafc' }}>
                    <TableCell sx={{ fontWeight: 600, color: '#0f172a' }}>Date & Time</TableCell>
                    <TableCell sx={{ fontWeight: 600, color: '#0f172a' }}>Event</TableCell>
                    <TableCell sx={{ fontWeight: 600, color: '#0f172a' }}>Value</TableCell>
                    <TableCell sx={{ fontWeight: 600, color: '#0f172a' }}>ML Insight</TableCell>
                    <TableCell sx={{ fontWeight: 600, color: '#0f172a' }}>Severity</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {paginatedData.map((row, i) => (
                    <TableRow key={i} sx={{ '&:hover': { bgcolor: '#f8fafc' } }}>
                      <TableCell>
                        <Typography variant="body2" sx={{ color: '#0f172a', fontWeight: 500 }}>{row.date}</Typography>
                        <Typography variant="caption" sx={{ color: '#64748b' }}>{row.time}</Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2" sx={{ color: '#0f172a' }}>{row.event}</Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2" sx={{ fontWeight: 600, color: PRIMARY_GREEN }}>{row.value}</Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="caption" sx={{ color: '#475569' }}>{row.insight}</Typography>
                      </TableCell>
                      <TableCell>
                        <Chip
                          label={row.severity}
                          size="small"
                          sx={{ bgcolor: `${row.color}15`, border: `1px solid ${row.color}30`, color: row.color, fontWeight: 600 }}
                        />
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        {/* ML Prediction vs Actual Usage Chart */}
        <Grid size={{ xs: 12, lg: 7 }}>
          <Paper sx={{ p: 3, border: '1px solid #e2e8f0' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
              <Avatar sx={{ bgcolor: `rgba(13,107,58,0.1)`, width: 40, height: 40 }}>
                <WaterDropIcon sx={{ color: PRIMARY_GREEN }} />
              </Avatar>
              <Box>
                <Typography variant="h6" sx={{ fontWeight: 700, color: '#0f172a' }}>
                  ML Prediction vs Actual Usage
                </Typography>
                <Typography variant="caption" sx={{ color: '#64748b' }}>
                  How accurate is our ML model? Comparing predicted vs actual water usage
                </Typography>
              </Box>
            </Box>
            ...
            <Box sx={{ height: 320 }}>
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={mlAccuracyData}>
                  ...
                  <YAxis tick={{ fill: '#475569' }} label={{ value: 'Water Usage (Liters)', angle: -90, position: 'insideLeft', fill: '#475569' }} />
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="predicted" name="ML Predicted" fill={PRIMARY_GREEN} radius={[6, 6, 0, 0]} barSize={40} />
                  <Bar dataKey="actual" name="Actual Used" fill="#f97316" radius={[6, 6, 0, 0]} barSize={40} />
                </ComposedChart>
              </ResponsiveContainer>
            </Box>
            ...
            <Box sx={{ mt: 2, p: 2, borderRadius: 2, bgcolor: '#f8fafc' }}>
              <Typography variant="caption" sx={{ color: PRIMARY_GREEN, fontWeight: 600 }}>
                ML Model Accuracy: {overallAccuracy}% · Predicted less by {savingsAmount.toLocaleString()} L ({savingsPercent}% savings)
              </Typography>
            </Box>
          </Paper>
        </Grid>
        ...
        <Grid size={{ xs: 12, lg: 5 }}>
          <Paper sx={{
            ...
            position: 'relative',
            '&:hover': { transform: 'translateY(-4px)', boxShadow: `0 12px 24px -12px rgba(13,107,58,0.2)` }
          }}>
            <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: '3px', background: `linear-gradient(90deg, ${PRIMARY_GREEN}, #f97316)` }} />

            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
              <Avatar sx={{ bgcolor: `rgba(13,107,58,0.1)`, width: 48, height: 48 }}>
                <AutoGraphIcon sx={{ color: PRIMARY_GREEN }} />
              </Avatar>
              <Box>
                <Typography variant="h6" sx={{ fontWeight: 700, color: '#0f172a' }}>
            ...
            <Box sx={{ textAlign: 'center', mb: 3 }}>
              <Typography
                ...
                sx={{
                  fontSize: '3.5rem',
                  fontWeight: 800,
                  background: `linear-gradient(135deg, ${PRIMARY_GREEN}, #f97316)`,
            ...
            <Grid container spacing={2} sx={{ mb: 3 }}>
              <Grid size={6}>
                <Box sx={{ p: 2, borderRadius: 2, bgcolor: `rgba(13,107,58,0.04)` }}>
                  <Typography variant="caption" sx={{ color: '#64748b' }}>Total Predicted</Typography>
                  <Typography variant="h5" sx={{ fontWeight: 700, color: PRIMARY_GREEN }}>{totalPredicted.toLocaleString()} L</Typography>
                </Box>
              </Grid>
              <Grid size={6}>
                <Box sx={{ p: 2, borderRadius: 2, bgcolor: `rgba(249,115,22,0.04)` }}>
                  <Typography variant="caption" sx={{ color: '#64748b' }}>Total Actual</Typography>
                  <Typography variant="h5" sx={{ fontWeight: 700, color: '#f97316' }}>{totalActual.toLocaleString()} L</Typography>
                </Box>
              </Grid>
            </Grid>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <Button fullWidth variant="outlined" startIcon={<PictureAsPdfIcon />} sx={{ py: 1.2, borderColor: '#f97316', color: '#f97316', '&:hover': { transform: 'translateY(-2px)' } }}>
                PDF Report
              </Button>
            </Box>
          </Paper>

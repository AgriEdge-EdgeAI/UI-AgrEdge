...
const Register: React.FC = () => {
  ...
  return (
    <Container maxWidth="md" sx={{ minHeight: '80vh', display: 'flex', alignItems: 'center', py: 4 }}>
      <Paper elevation={3} sx={{ p: 4, width: '100%', borderRadius: 4, border: '1px solid #e2e8f0', bgcolor: '#ffffff' }}>
        <Box sx={{ textAlign: 'center', mb: 3 }}>
          <Avatar sx={{ mx: 'auto', mb: 2, bgcolor: PRIMARY_GREEN, width: 56, height: 56 }}>
            <AgricultureIcon sx={{ fontSize: 28, color: '#fff' }} />
          </Avatar>
          <Typography variant="h4" sx={{ fontFamily: '"Space Grotesk", sans-serif', fontWeight: 700, color: '#0f172a' }}>Create Account</Typography>
          <Typography variant="body2" sx={{ color: '#475569', mt: 1 }}>Join AgriEdge and start saving water today</Typography>
        </Box>
        ...
        <form onSubmit={handleRegister}>
          ...
          <Box sx={{ textAlign: 'center', mt: 3 }}>
            <Typography variant="body2" sx={{ color: '#475569' }}>Already have an account?{' '}
              <Link component="button" underline="hover" sx={{ color: PRIMARY_GREEN, fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/login')}>Sign In</Link>
            </Typography>
          </Box>
        </form>
      </Paper>
